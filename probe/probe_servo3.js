// Multi-servo probe v3: subscribe to notifications and log responses to mirror working behavior.
  ...
  for (const nc of notifyChars) {
    try {
      ...
      nc.on('data', (d) => {
        ...
        if (pkt.cmd === 0x09 && pkt.params.length === 1) {
          const ackByte = pkt.params[0];
          const pretty = util.inspect(['OK', `char=${nc.uuid}`, 'cmd=0x09', `data=${ackByte.toString(16).padStart(2, '0')}`], {
            colors: true,
            compact: true,
        ...
        if (pkt.cmd === 0x0b) {
          ...
          console.log(`POS char=${nc.uuid} id=${pos?.id ?? '?'} val=${pos?.value ?? '?'} raw=${pos?.rawHex ?? '??'}`);
        } else {
          const paramsHex = Buffer.from(pkt.params).toString('hex');
          const shortHex = paramsHex.length > 16 ? `${paramsHex.slice(0, 16)}...` : paramsHex || 'none';
          console.log(`NTFY char=${nc.uuid} cmd=0x${pkt.cmd.toString(16)} len=${pkt.params.length} data=${shortHex}`);
        }
      });
    } catch (e) {
